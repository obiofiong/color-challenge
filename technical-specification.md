...
| | `updateApplicationStatus` | ✅ | Updates `event_applications.status` + `reviewed_at`/`reviewed_by`; **on `approved`, also inserts a new `contestants` row** (`color: ''` default, error now checked — see §8 #8) linked via `application_id` (this is the only place a contestant is created outside the admin "Add Contestant" form) |
...
8. **`contestants.color` is NOT NULL**, and `updateApplicationStatus`'s auto-create-contestant-on-approval path only inserted `name`/`bio`. Every approval silently failed to create a contestant (the insert error was never checked) — the application flipped to `approved` with nothing else happening. Fixed by supplying a `color: ''` default and checking the insert error. **Any other code path that inserts into `contestants` must supply `color`, not just the nullable columns.**
